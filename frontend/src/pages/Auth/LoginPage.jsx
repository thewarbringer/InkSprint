...
export default function LoginPage() {
  ...
  const handleGoogleCredential = useCallback(
    async (response) => {
      setSubmitError(null);
      try {
        const res = await fetch(`${getApiBaseUrl()}/api/auth/google-signin`, {
          method: "POST",
        ...
        if (!res.ok) {
          ...
          setSubmitError(errorData.message || "Google sign-in failed.");
          return;
        }
  ...
  async function onSubmit(data) {
    ...
    try {
      ...
      if (!response.ok) {
        ...
        setSubmitError(errorData.message || "Invalid email or password.");
        return;
      }

      const result = await response.json();

      setUserSession(result, data.remember);
    ...
    >
      <form onSubmit={handleSubmit(onSubmit)} noValidate>
        ...
        <div className="mb-6 flex items-center justify-between">
          ...
          <Link
            to="/forgot-password"
            className="text-[13.5px] text-secondary hover:underline"
          >
            Forgot password?
          </Link>
        </div>
        ...
        {submitError && (
          <p className="mb-4 text-[13.5px] text-danger">
            {submitError}
          </p>
        )}
